...
use amalgam_codegen::{nickel::NickelCodegen, test_debug::TestDebugCapture, Codegen};
use amalgam_core::{
...
fn test_import_pipeline_with_debug_validation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        generated.contains("# Module: k8s.io.v1") || generated.contains("k8s.io.v1"),
    ...
    assert!(
        generated.contains("import")
            || generated.contains("Container")
            || generated.contains("Lifecycle"),
...
fn test_same_module_import_resolution() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut codegen = NickelCodegen::from_ir(&ir).with_debug_config(capture.config().clone());
    ...
    assert!(
        generated.contains("TypeB"),
        "Should contain TypeB reference"
    );
    // But should NOT have an import for it
    assert!(
        !generated.contains("TypeB.ncl"),
        "Should NOT import TypeB when it's in the same module"
    );
    Ok(())
}
...
fn test_underscore_module_name_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        generated == "String" || generated.contains("String"),
