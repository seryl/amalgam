...
impl NickelManifestGenerator {
    ...
    fn has_reference_to(&self, ty: &amalgam_core::types::Type, prefix: &str) -> bool {
        ...
        match ty {
            ...
            Type::Array(inner) | Type::Optional(inner) => self.has_reference_to(inner, prefix),
            Type::Map { value, .. } => self.has_reference_to(value, prefix),
            Type::Record { fields, .. } => fields
                .values()
                .any(|f| self.has_reference_to(&f.ty, prefix)),
            Type::Union { types, .. } => types.iter().any(|t| self.has_reference_to(t, prefix)),
            Type::TaggedUnion { variants, .. } => {
                variants.values().any(|t| self.has_reference_to(t, prefix))
            }
            _ => false,
        }
    }
