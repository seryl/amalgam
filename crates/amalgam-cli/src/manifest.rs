...
impl Manifest {
  ...
  description = {},
    ...
    /// Recursively generate mod.ncl for a directory with rich metadata
    fn generate_mod_ncl_for_dir(&self, dir: &Path) -> Result<()> {
        ...
        if !subdirs.is_empty() || !files.is_empty() {
            let mod_content = self.generate_rich_mod_ncl(dir, &subdirs, &files)?;
    ...
    fn generate_rich_mod_ncl(
        ...
        // Extract module context from path
        let rel_path = dir.strip_prefix(&self.config.output_base)
            .unwrap_or(dir);
        ...
        content.push_str(&format!(
            ...
            if path_parts.is_empty() {
                ...
                format!("Module: {}", path_parts.join("."))
            }
        ));
        content.push_str("# Auto-generated by Amalgam\n");
        content.push_str(&format!("# Generated: {}\n", chrono::Utc::now().format("%Y-%m-%d %H:%M:%S UTC")));

        // Add package context if at root
        if path_parts.is_empty() {
            content.push_str("#\n");
            content.push_str("# This package contains type-safe Nickel definitions generated from\n");
            content.push_str("# Kubernetes CRDs, OpenAPI schemas, or other type sources.\n");
            content.push_str("#\n");
            content.push_str("# Usage:\n");
            content.push_str("#   let types = import \"./mod.ncl\" in\n");
            content.push_str("#   types.SomeType\n");
        } else {
            ...
            if path_parts.len() == 1 {
                content.push_str("# API Group - Contains all versions for this group\n");
            } else if path_parts.len() == 2 {
                content.push_str("# Version Module - Contains all types for this version\n");
            }
        }

        content.push_str("\n");

        // Add type exports list as documentation
        if !files.is_empty() {
        ...
        // Add submodule list as documentation
        if !subdirs.is_empty() {
        ...
        // Add cross-references if this looks like a k8s package
        let package_name = path_parts.first().map(|s| s.to_string());
        if let Some(pkg) = &package_name {
            if pkg.contains("k8s") || pkg.contains("crossplane") {
                content.push_str("# Related packages:\n");
                if pkg.contains("crossplane") && !pkg.contains("k8s") {
                    content.push_str("#   - k8s_io (Kubernetes core types)\n");
                }
                content.push_str("#   See ../Nickel-pkg.ncl for dependencies\n");
                content.push_str("#\n");
            }
        }
        ...
        // Add subdirectory imports with documentation
        ...
        // Add file imports with documentation
