...
pub async fn handle_k8s_core_import(
    ...
    for (type_ref, type_def) in types_map {
        ...
        let api_group = if type_ref.group.starts_with("io.k8s.api.") {
            ...
            let group_part = type_ref.group.strip_prefix("io.k8s.api.")
            ...
            format!("k8s.io.apimachinery.{}", 
                type_ref.group.strip_prefix("io.k8s.apimachinery.")
                    .unwrap_or(""))
        } else {
    ...
    for api_group in &all_groups {
        ...
        info!("Processing API group {} with {} versions", api_group, versions.len());
        ...
        for version_name in versions {
            ...
            let version_dir = if api_group == "k8s.io" {
                ...
                let group_part = api_group.strip_prefix("k8s.io.")
                    .unwrap_or(api_group);
                output_dir.join(group_part).join(&version_name)
            } else {
                // Fallback for any other pattern
                output_dir.join(api_group.replace('.', "/")).join(&version_name)
            };
        ...
        for api_group in &all_groups {
            ...
            if api_group.starts_with("k8s.io.") {
                ...
                if let Some(versions) = package.types.get(api_group) {
                    for version in versions.keys() {
                        group_imports.push(format!("  {} = import \"./{}/mod.ncl\",", version, version));
                    }
                }
                ...
                // Add to root imports
                root_imports.push(format!("  {} = import \"./{}/mod.ncl\",", group_part, group_part));
            }
        }
