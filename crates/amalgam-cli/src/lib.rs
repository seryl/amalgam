...
pub async fn handle_k8s_core_import(version: &str, output_dir: &PathBuf, nickel_package: bool) -> Result<()> {
    ...
    for (version, version_types) in &types_by_version {
        ...
        let type_names: Vec<String> = version_types.iter().map(|(tr, _)| tr.kind.clone()).collect();
        let mut mod_content = String::from("# Module exports for this version\n{\n");
        for type_name in &type_names {
            let file_name = type_name.to_lowercase();
            mod_content.push_str(&format!("  {} = import \"./{}.ncl\",\n", type_name, file_name));
        }
        mod_content.push_str("}\n");
        fs::write(version_dir.join("mod.ncl"), mod_content)?;
        
        info!("Generated {} types for version {}", version_types.len(), version);
    }
    ...
    if nickel_package {
        ...
        for version in types_by_version.keys() {
            mod_content.push_str(&format!("  {} = import \"./{}/mod.ncl\",\n", version, version));
        }
        mod_content.push_str("}\n");
        fs::write(output_dir.join("mod.ncl"), mod_content)?;
    }
    
    info!("✓ Successfully generated {} Kubernetes core types", total_types);
    Ok(())
}