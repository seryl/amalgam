...
fn validate_single_file_with_package_path(nickel_binary: &str, file: &Path, package_path: Option<&Path>) -> Result<()> {
...
fn validate_package_with_package_path(nickel_binary: &str, dir: &Path, package_path: Option<&Path>) -> Result<()> {
            ...
            match pkg_check {
                ...
                Ok(output) => {
                    ...
                    if stderr.contains("unrecognized subcommand") {
                        ...
                        validate_single_file_with_package_path(nickel_binary, pkg_manifest, package_path)?;
                    } else {
