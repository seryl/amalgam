...
fn test_type_with_multiple_same_module_deps() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(output.contains("EphemeralContainer"), "Should have EphemeralContainer type");
...
fn test_cross_version_import_chain() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(output.contains("BetaType") || output.contains("coreRef"), "Should have BetaType or its fields");
    assert!(output.contains("AlphaType") || output.contains("betaRef"), "Should have AlphaType or its fields");
