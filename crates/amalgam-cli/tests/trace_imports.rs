...
async fn trace_k8s_imports() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let subscriber = tracing_subscriber::registry().with(forest_layer).with(
        tracing_subscriber::EnvFilter::from_default_env()
            .add_directive("amalgam=trace".parse().map_err(|e| format!("Parse error: {}", e))?)
            .add_directive("amalgam_parser=trace".parse().map_err(|e| format!("Parse error: {}", e))?)
            .add_directive("amalgam_core=trace".parse().map_err(|e| format!("Parse error: {}", e))?)
            .add_directive("amalgam_codegen=trace".parse().map_err(|e| format!("Parse error: {}", e))?),
    );
    ...
    if v1alpha1_vac.exists() {
        let content = fs::read_to_string(&v1alpha1_vac)?;
