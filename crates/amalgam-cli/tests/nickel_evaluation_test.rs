...
fn test_generated_k8s_packages_evaluate() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_file = examples_dir
        .join("fixtures")
        .join("nickel")
        .join("test_k8s_pod.ncl");
    ...
    assert!(
        stdout.contains(r#""apiVersion": "v1""#),
        "Output should contain apiVersion"
    );
    assert!(
        stdout.contains(r#""kind": "Pod""#),
        "Output should contain Pod kind"
    );
    assert!(
        stdout.contains(r#""test-pod""#),
        "Output should contain test-pod name"
    );
...
fn test_cross_package_imports() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_file = examples_dir
        .join("fixtures")
        .join("nickel")
        .join("test_crossplane_with_k8s.ncl");
...
fn test_import_naming_conventions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_file = examples_dir
        .join("fixtures")
        .join("nickel")
        .join("test_naming_conventions.ncl");
