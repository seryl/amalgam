...
use amalgam::manifest::{
    DependencySpec, ManifestConfig, ManifestGenerator, PackageDefinition, SourceType,
};
...
fn test_manifest_with_index_dependencies() {
    ...
    let packages = vec![
        ...
        PackageDefinition {
            ...
            dependencies: {
                ...
                deps.insert(
                    "k8s_io".to_string(),
                    DependencySpec::Simple("1.31.0".to_string()),
                );
                deps
            },
            enabled: true,
    ...
    fs::write(output_base.join("k8s_io/mod.ncl"), "{ test = \"k8s\" }").unwrap();
    ...
    generator
        .generate_package_manifest(crossplane_pkg, &output_base.join("crossplane"))
        .unwrap();
    ...
    assert!(
        manifest_content
            .contains("'Index { package = \"github:test/packages/k8s-io\", version = \"1.31.0\" }"),
        "Should use Index dependency format"
    );

    // Should NOT contain Path dependencies
    assert!(
        !manifest_content.contains("'Path"),
        "Should not contain Path dependencies"
    );

    // Verify metadata comments
    assert!(
        manifest_content.contains("# Generated:"),
        "Should have generation timestamp"
    );
    assert!(
        manifest_content.contains("# Git ref: v1.17.2"),
        "Should have git ref"
    );
    assert!(
        manifest_content.contains("# Generator: amalgam"),
...
fn test_manifest_without_local_prefix() {
    ...
    fs::write(output_base.join("pkg_a/mod.ncl"), "{ test = \"a\" }").unwrap();
    ...
    generator
        .generate_package_manifest(pkg_b, &output_base.join("pkg_b"))
        .unwrap();
    ...
    assert!(
        manifest_content
            .contains("'Index { package = \"github:org/repo/pkg-a\", version = \"1.0.0\" }"),
...
fn test_auto_detect_dependencies() {
    ...
    generator
        .generate_package_manifest(derived_pkg, &output_base.join("derived"))
        .unwrap();
    ...
    assert!(
        manifest_content.contains("base = 'Index { package = \"github:test/pkgs/base\""),
...
fn test_manifest_with_git_ref() {
    ...
    generator
        .generate_package_manifest(pkg, &output_base.join("versioned_pkg"))
        .unwrap();

    let manifest_content =
        fs::read_to_string(output_base.join("versioned_pkg/Nickel-pkg.ncl")).unwrap();

    // Verify git ref is in metadata comments
    assert!(
        manifest_content.contains("# Git ref: v3.2.1"),
        "Should include git ref in metadata comments"
    );

    // Verify version field
    assert!(
        manifest_content.contains("version = \"3.2.1\""),
...
fn test_package_resolution_from_manifest() {
    ...
    generator
        .generate_package_manifest(consumer_pkg, &output_base.join("consumer"))
        .unwrap();
    ...
    assert!(
        manifest_content.contains("'Index { package = \"github:myorg/packages/special-name\""),
...
mod validation_tests {
    ...
    fn test_validate_generated_manifest() {
        ...
        let pkg = &generator.packages[0];
        generator
            .generate_package_manifest(pkg, &output_base.join("simple"))
            .unwrap();
