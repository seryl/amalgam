...
fn main() {
    ...
    let k8s_version = if manifest_path.exists() {
        ...
        // Use default version when manifest is missing (e.g., during initial setup)
        eprintln!("Warning: .amalgam-manifest.toml not found, using default k8s version v1.33.4");
        "v1.33.4".to_string()
    };
