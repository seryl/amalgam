...
use crate::{Parser, ParserError, imports::ImportResolver, k8s_authoritative::K8sTypePatterns};
use amalgam_core::{ir::{IR, IRBuilder}, types::Type};
...
impl CRDParser {
    ...
    pub fn parse_version(&self, crd: &CRD, version_name: &str) -> Result<IR, ParserError> {
        ...
        let version = crd.spec.versions.iter()
            .find(|v| v.name == version_name)
            .ok_or_else(|| ParserError::Parse(
                format!("Version {} not found in CRD", version_name)
            ))?;
        ...
        if let Some(schema) = &version.schema {
            ...
            Err(ParserError::Parse(
                format!("Version {} has no schema", version_name)
            ))
        }
    }
        ...
        if let Type::Record { mut fields, open } = ty {
            ...
            if let Some(metadata_field) = fields.get_mut("metadata") {
                if matches!(metadata_field.ty, Type::Record { ref fields, .. } if fields.is_empty()) {
                    metadata_field.ty = Type::Reference("io.k8s.apimachinery.pkg.apis.meta.v1.ObjectMeta".to_string());
                }
            }
            ...
            if let Some(status_field) = fields.get_mut("status") {
                ...
                if let Type::Record { fields: ref mut status_fields, .. } = &mut status_field.ty {
    ...
    fn enhance_field_type_with_context(&self, ty: Type, context: &[&str]) -> Result<Type, ParserError> {
        match ty {
            Type::Record { fields, open } => {
                ...
                for (field_name, field) in enhanced_fields.iter_mut() {
                    ...
                    if let Some(go_type) = self.k8s_patterns.get_contextual_type(field_name, context) {
                        ...
                        let should_replace = match (field_name.as_str(), &field.ty, go_type.as_str()) {
                            // Metadata should be ObjectMeta if it's currently empty
                            ("metadata", Type::Record { fields, .. }, _) if fields.is_empty() => true,
                    ...
                    field.ty = self.enhance_field_type_with_context(field.ty.clone(), &new_context)?;
                }
            ...
            Type::Array(inner) => {
                Ok(Type::Array(Box::new(self.enhance_field_type_with_context(*inner, context)?)))
            }
            Type::Optional(inner) => {
                Ok(Type::Optional(Box::new(self.enhance_field_type_with_context(*inner, context)?)))
            }
            _ => Ok(ty)
        }
    }
            ...
            Some("object") => {
                ...
                if let Some(Value::Object(props)) = schema.get("properties") {
                    ...
                    for (name, prop_schema) in props {
                        ...
                        fields.insert(name.clone(), amalgam_core::types::Field {
                            ty,
