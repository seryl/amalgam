...
fn parse_swagger_json_lenient(json: &Value) -> Result<IR, ParserError> {
    ...
    if let Some(definitions) = json.get("definitions").and_then(|d| d.as_object()) {
        ...
        let mut modules: std::collections::HashMap<String, Vec<(String, Type)>> = std::collections::HashMap::new();
...
/// e.g., "io.k8s.api.core.v1.Pod" -> ("api.core.v1", "Pod")
fn parse_k8s_type_name(full_name: &str) -> (String, String) {
    ...
    if full_name.starts_with("io.k8s.") {
        let without_prefix = full_name.strip_prefix("io.k8s.").unwrap();
    ...
    match schema.get("type").and_then(|t| t.as_str()) {
        ...
        Some("array") => {
            let item_type = schema
                .get("items")
                .map(|items| json_schema_to_type(items))
                .transpose()?
                .unwrap_or(Type::Any);
                    ...
                    fields.insert(
                        ...
                        Field {
                            ...
                            description: field_schema
                                ...
                                .and_then(|d| d.as_str())
                                .map(String::from),
                            default: field_schema.get("default").and_then(|v| serde_json::from_value(v.clone()).ok()),
                        },
                    );
                }
            }
