...
impl NamespacedPackage {
    ...
    /// Generate the main module file with rich documentation
    pub fn generate_main_module(&self) -> String {
        let mut content = String::new();

        // Header with package information
        content.push_str(&format!("# Package: {}\n", self.name));
        content.push_str("# Auto-generated by Amalgam\n");
        content.push_str(&format!("# Generated: {}\n", chrono::Utc::now().format("%Y-%m-%d %H:%M:%S UTC")));
        content.push_str("#\n");
        content.push_str("# This package contains type-safe Nickel definitions for Kubernetes types.\n");
        content.push_str("# Each API group is organized into versions, with individual type definitions.\n");
        content.push_str("#\n");
        content.push_str("# Structure: group > version > kind\n");
        content.push_str("#\n");
        content.push_str("# Usage:\n");
        content.push_str("#   let pkg = import \"./mod.ncl\" in\n");
        content.push_str("#   pkg.<group>.<version>.<Kind>\n");
        content.push_str("#\n");
        ...
        content.push_str("# Available API Groups:\n");
        for group in &groups {
            let versions = self.types.get(*group).map(|v| v.keys().collect::<Vec<_>>());
            if let Some(vers) = versions {
                content.push_str(&format!("#   - {} (versions: {})\n",
                    group,
                    vers.iter().map(|v| v.as_str()).collect::<Vec<_>>().join(", ")
                ));
            }
        }
        content.push_str("\n");
    ...
    /// Generate a group-level module file with rich documentation
    pub fn generate_group_module(&self, group: &str) -> Option<String> {
        self.types.get(group).map(|versions| {
            ...
            content.push_str(&format!("# Generated: {}\n", chrono::Utc::now().format("%Y-%m-%d %H:%M:%S UTC")));
            content.push_str("#\n");
            content.push_str("# This module contains all versions for this API group.\n");
            content.push_str("#\n");

            // List versions and their types
            let mut version_list: Vec<_> = versions.keys().collect();
            version_list.sort();

            content.push_str("# Available Versions:\n");
            for version in &version_list {
                if let Some(types) = versions.get(*version) {
                    let type_names: Vec<_> = types.keys().collect();
                    content.push_str(&format!("#   - {} ({} types)\n", version, type_names.len()));
                }
            }
            content.push_str("#\n");
            content.push_str("# Usage:\n");
            content.push_str(&format!("#   let group = import \"./{}/mod.ncl\" in\n", group));
            content.push_str("#   group.<version>.<Type>\n");
            content.push_str("\n");
    ...
    /// Generate a version-level module file for a group with rich documentation
    pub fn generate_version_module(&self, group: &str, version: &str) -> Option<String> {
        self.types.get(group).and_then(|versions| {
            versions.get(version).map(|kinds| {
                ...
                content.push_str(&format!("# Generated: {}\n", chrono::Utc::now().format("%Y-%m-%d %H:%M:%S UTC")));
                content.push_str("#\n");
                content.push_str("# This module exports all type definitions for this version.\n");
                content.push_str("#\n");
                ...
                content.push_str("#\n");
                content.push_str("# Usage:\n");
                content.push_str(&format!("#   let v = import \"./{}/{}/mod.ncl\" in\n", group, version));
                content.push_str("#   v.<TypeName>\n");
                content.push_str("\n");
    ...
    pub fn generate_version_files(&self, group: &str, version: &str) -> BTreeMap<String, String> {
            ...
            if let Some(module) = module {
                ...
                for type_def in &module.types {
                    ...
                    let (content, strategy, success, error) = if module.types.len() == 1 {
                        eprintln!("🛤️ PATH: Using single-type extraction for '{}'", type_def.name);
                        ...
                        eprintln!("🛤️ PATH: Using multi-type extraction for '{}' (module has {} types)", type_def.name, module.types.len());
        ...
        if files.is_empty() {
            eprintln!("🚨 FALLBACK: Using fallback generation for group '{}' version '{}'", group, version);
...
fn extract_single_type_from_module(
    ...
    eprintln!("🔧 EXTRACTION: Processing type '{}' in extract_single_type_from_module", type_name);
    ...
    if let Some(import_map) = type_import_map {
        ...
        eprintln!("📥 EXTRACTION: TypeImportMap check for '{}': found {} imports", type_name, map_imports.len());
        if !map_imports.is_empty() {
            eprintln!("📋 EXTRACTION: Import map contains for type '{}': {:?}", type_name, map_imports);
