...
impl K8sTypesFetcher {
    ...
    pub async fn fetch_k8s_openapi(&self, version: &str) -> Result<Value, ParserError> {
        ...
        if !response.status().is_success() {
            if let Some(pb) = pb {
                pb.finish_with_message(format!("✗ Failed to fetch k8s OpenAPI: {}", response.status()));
            }
    ...
    fn parse_type_reference(&self, full_name: &str) -> Result<TypeReference, ParserError> {
        ...
        if parts.len() < 5 || parts[0] != "io" || parts[1] != "k8s" {
            return Err(ParserError::Parse(format!("Invalid k8s type name: {}", full_name)));
        }
    ...
    fn schema_to_type_definition(&self, name: &str, schema: &Value) -> Result<TypeDefinition, ParserError> {
        let ty = self.json_schema_to_type(schema)?;
        
        Ok(TypeDefinition {
            name: name.to_string(),
            ty,
            documentation: schema.get("description")
                .and_then(|d| d.as_str())
                .map(String::from),
    ...
    let object_meta = TypeDefinition {
        ...
        ty: Type::Record {
            fields: {
                ...
                fields.insert("namespace".to_string(), Field {
                    ty: Type::Optional(Box::new(Type::String)),
                    required: false,
                    description: Some("Namespace defines the space within which each name must be unique".to_string()),
                    default: None,
                });
                fields.insert("labels".to_string(), Field {
                    ty: Type::Optional(Box::new(Type::Map {
                        key: Box::new(Type::String),
                        value: Box::new(Type::String),
                    })),
                    required: false,
                    description: Some("Map of string keys and values for organizing and categorizing objects".to_string()),
                    default: None,
                });
                fields.insert("annotations".to_string(), Field {
                    ty: Type::Optional(Box::new(Type::Map {
                        key: Box::new(Type::String),
                        value: Box::new(Type::String),
                    })),
                    required: false,
                    description: Some("Annotations is an unstructured key value map".to_string()),
                    default: None,
                });
                fields.insert("uid".to_string(), Field {
                    ty: Type::Optional(Box::new(Type::String)),
                    required: false,
                    description: Some("UID is the unique in time and space value for this object".to_string()),
                    default: None,
                });
                fields.insert("resourceVersion".to_string(), Field {
                    ty: Type::Optional(Box::new(Type::String)),
                    required: false,
                    description: Some("An opaque value that represents the internal version of this object".to_string()),
                    default: None,
                });
                fields
            },
        ...
        documentation: Some("ObjectMeta is metadata that all persisted resources must have".to_string()),
        annotations: HashMap::new(),
    };
