...
impl K8sTypesFetcher {
    ...
    fn parse_type_reference(&self, full_name: &str) -> Result<TypeReference, ParserError> {
        ...
        // Always use k8s.io as the group for all k8s types
        // We organize them by version, not by API group
        let group = "k8s.io".to_string();
    ...
    fn schema_to_type_definition(
        ...
        // Special case for IntOrString - it's defined as a string in OpenAPI but should be a union
        let ty = if name == "IntOrString" {
            Type::Union {
                types: vec![Type::Integer, Type::String],
                coercion_hint: Some(amalgam_core::types::UnionCoercion::PreferString),
            }
        } else {
            self.json_schema_to_type(schema)?
        };
