...
impl CRDFetcher {
    ...
    async fn fetch_from_github(&self, url: &str, is_tty: bool) -> Result<Vec<CRD>> {
        ...
        let main_progress = if is_tty {
            let pb = self.multi_progress.add(ProgressBar::new(yaml_files.len() as u64));
            pb.set_style(
        ...
        if let Some(ref main_pb) = main_progress {
            main_pb.finish_with_message(format!("✓ Successfully downloaded {} valid CRDs", crds.len()));
        } else {
    ...
    async fn fetch_directory(&self, _url: &str) -> Result<Vec<CRD>> {
        ...
        Err(anyhow::anyhow!("Directory listing not supported for non-GitHub URLs"))
    }
...
async fn fetch_single_crd(client: &reqwest::Client, url: &str) -> Result<CRD> {
    ...
    if value.get("kind") == Some(&serde_yaml::Value::String("CustomResourceDefinition".to_string())) {
