...
fn test_objectmeta_file_structure() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let has_managed_fields_import = content.lines().any(|l| {
        l.contains("let managedFieldsEntry = import") && l.contains("ManagedFieldsEntry.ncl")
    });

    let has_owner_ref_import = content
        .lines()
...
# We test coordination/v1alpha2 to have consistent error reporting
let v1alpha2 = import "examples/pkgs/k8s_io/api/coordination/v1alpha2.ncl" in

{
  # This will fail with a consistent error about missing imports
  test_result = "Testing comprehensive import"
}
"#;
    ...
    let normalized_output = output
        ...
        .map(|line| {
            ...
            if line.contains("test_comprehensive_temp_") {
                ...
                re.replace_all(line, "test_comprehensive_temp.ncl")
                    .to_string()
            } else {
                line.to_string()
            }
...
# This ensures the snapshot test is deterministic
let v1 = import "examples/pkgs/k8s_io/api/core/v1.ncl" in

{
  # This will fail due to missing import, but it will fail consistently
  test_result = "Testing import"
}
"#;
