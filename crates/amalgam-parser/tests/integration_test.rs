...
"#;
    ...
    assert_eq!(ir.modules.len(), 1, "Should have 1 module for single version");
    ...
    let nickel_code = codegen.generate(&ir).expect("Failed to generate Nickel code");
...
"#;
    ...
    let package = generator.generate_package().expect("Failed to generate package");
...
"#;
    ...
    let complex_module = ir.modules.iter()
        .find(|m| m.name.contains("Complex"))
        .expect("Complex module not found");
    
    let complex_type = complex_module.types.iter()
...
"#;
    ...
    let crd = load_test_crd(crd_yaml);
    let parser = CRDParser::new();
    let ir = parser.parse(crd.clone()).expect("Failed to parse multi-version CRD");
    ...
    let module_names: Vec<String> = ir.modules.iter()
        .map(|m| m.name.clone())
        .collect();
    
    assert!(module_names.iter().any(|n| n.contains("v1alpha1")), 
            "Should have v1alpha1 module");
    assert!(module_names.iter().any(|n| n.contains("v1beta1")), 
            "Should have v1beta1 module");
    assert!(module_names.iter().any(|n| n.contains(".v1.")), 
...
"#;
    ...
    assert!(package.generate_kind_file("test.io", "v1alpha1", "evolving").is_some());
    assert!(package.generate_kind_file("test.io", "v1beta1", "evolving").is_some());
    assert!(package.generate_kind_file("test.io", "v1", "evolving").is_some());
}
