                    ...
                    fields.insert(
                        ...
                        Field {
                            ty: Type::Optional(Box::new(Type::Reference {
                                name: "CELDeviceSelector".to_string(),
                                module: None, // Same package reference
                            })),
                            required: false,
...
fn test_v1alpha3_same_version_imports() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Build registry and dependencies with non-k8s.io package
    let registry = PackageWalkerAdapter::build_registry(&types, "example.io", "v1alpha3")?;
    let deps = PackageWalkerAdapter::build_dependencies(&registry);

    // Generate IR
    let ir = PackageWalkerAdapter::generate_ir(registry, deps, "example.io", "v1alpha3")?;
...
fn test_import_path_calculator_same_version() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test same-version imports with non-k8s.io packages
    ...
    let path = calc.calculate("example.io", "v1", "example.io", "v1", "pod");
