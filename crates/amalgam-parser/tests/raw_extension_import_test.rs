...
fn test_rawextension_v0_import() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for module in &ir.modules {
        for import in &module.imports {
            ...
            if import.path.contains("apimachinery.pkg.apis/meta") {
                // ObjectMeta should import from consolidated apimachinery module
                assert!(
                    import
                        .path
                        .contains("apimachinery.pkg.apis/meta/v1/mod.ncl"),
                    "ObjectMeta should import from apimachinery consolidated module, got: {}",
                    import.path
                );
...
fn test_import_path_calculator_v0_imports() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test v1 -> v0 import for RawExtension (consolidated v0 module)
