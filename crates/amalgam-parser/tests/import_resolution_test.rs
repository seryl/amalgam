...
fn test_k8s_type_reference_detection() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Type::Record { fields, .. } = &type_def.ty {
        ...
        match &metadata_field.ty {
            ...
            _ => {
                return Err(format!("Unexpected type for metadata: {:?}", metadata_field.ty).into())
            }
        }
    } else {
...
fn test_reference_resolution_to_alias() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let module = Module {
        ...
        imports: vec![Import {
            path: "../../apimachinery.pkg.apis/meta/v1/mod.ncl".to_string(),
    ...
    assert!(
        generated.contains("let objectMeta = import")
...
fn test_case_insensitive_type_matching() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let module = Module {
        name: "test".to_string(),
        imports: vec![Import {
            // ObjectMeta is in consolidated apimachinery module
            path: "../../apimachinery.pkg.apis/meta/v1/mod.ncl".to_string(),
