...
impl SpecialCaseRegistry {
    ...
    pub fn default() -> Self {
        ...
        if let Ok(crossplane_rules) = Self::load_embedded_rules(include_str!("rules/crossplane.toml")) {
    ...
    pub fn get_import_override(&self, from_module: &str, target_type: &str) -> Option<&ImportOverride> {
        self.import_overrides.iter()
    ...
    fn apply_remapping(&self, module: &str, remapping: &ModuleRemapping) -> Option<String> {
        ...
        if remapping.from_pattern.contains("(.*)") || remapping.from_pattern.contains("(.+)") {
            ...
            if module.starts_with("io.k8s.api.") && remapping.from_pattern.starts_with("io.k8s.api.") {
                let rest = module.strip_prefix("io.k8s.api.")?;
                return Some(format!("api.{}", rest));
            }
            if module.starts_with("io.k8s.apimachinery.pkg.apis.") && remapping.from_pattern.starts_with("io.k8s.apimachinery.pkg.apis.") {
...
mod tests {
    ...
    fn test_type_transform() {
        ...
        let result = registry.transform_type_name(
