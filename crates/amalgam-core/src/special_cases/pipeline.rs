...
use super::{SpecialCaseRegistry, Context};
        ...
        match ty {
            Type::Record { fields, .. } => {
                ...
                for (field_name, field) in fields.iter() {
                    if let Some(new_name) = self.registry.get_field_rename(&context.current_module, field_name) {
            ...
            Type::Union { types, coercion_hint } => {
                ...
                if coercion_hint.is_none() {
                    ...
                    if let Some(strategy) = self.registry.get_coercion_strategy(&context.current_module) {
                        *coercion_hint = Some(match strategy {
                            super::CoercionStrategy::PreferString => crate::types::UnionCoercion::PreferString,
                            super::CoercionStrategy::PreferNumber => crate::types::UnionCoercion::PreferNumber,
...
mod tests {
    ...
    fn test_pipeline_field_rename() {
        ...
        fields.insert("$ref".to_string(), Field {
            ty: Type::String,
            required: true,
            description: None,
            default: None,
