...
impl ModuleStructure {
    ...
    fn is_version_dir(name: &str) -> bool {
        ...
        if let Some(num_end) = rest.find(|c: char| !c.is_ascii_digit()) {
            ...
            if !num_part.is_empty() && num_part.chars().all(|c| c.is_ascii_digit()) {
                if suffix.starts_with("alpha") || suffix.starts_with("beta") {
                    let version_suffix = if suffix.starts_with("alpha") {
                        &suffix[5..]
                    } else {
                        &suffix[4..]
                    };
    ...
    pub fn to_layout(&self) -> super::module_registry::ModuleLayout {
        ...
        let has_mixed_root = self.has_versions && self.namespaces.iter()
            .any(|ns| !Self::is_version_dir(ns));
        ...
        let parse_version = |v: &str| -> (u32, &str, u32) {
            ...
            if num_end < v.len() {
                ...
                if suffix.starts_with("alpha") {
                    let sub_version: u32 = suffix[5..].parse().unwrap_or(0);
                    return (main_version, "alpha", sub_version);
                } else if suffix.starts_with("beta") {
                    let sub_version: u32 = suffix[4..].parse().unwrap_or(0);
